...
class SkillsLoader:
    ...
    def split_triggered(self, names: list[str]) -> tuple[list[str], list[str]]:
        ...
        A skill whose frontmatter sets ``inject_on_trigger: true`` keeps the
        guaranteed-exposure semantics a pointer cannot offer: its procedure is
        in the prompt whether or not the agent chooses to read a file. That is
        the right trade only for a skill that must be *obeyed* on match — a
        mandatory pre-flight check, say — because it is charged in full on every
        turn the match repeats.

        It is the per-skill middle ground between the two extremes: ``always:
        true`` pays body price on every turn regardless of relevance, and
        ``$skillname`` requires the user to already know the skill exists.
        """
        enforced: list[str] = []
        ...
        for name in names:
            ...
            if skill_file is None:
                continue
            meta = self._cached_frontmatter(skill_file)
            if meta.get("inject_on_trigger", "").lower() == "true":
    ...
    def trigger_hint(self, names: list[str]) -> str:
        ...
        The counterpart to :meth:`get_triggered_skills`: the matcher decides
        which skills look relevant, and this renders that verdict as one line
        per skill instead of the skill's body. A body costs 8k-34k chars and is
        charged again on every turn the match repeats; a line costs ~150.
