...
async def _github_pull_request_state(ref: SourceRef) -> dict[str, Any]:
    ...
    return {"headSha": sha, "autoMergeArmed": auto is not None}
...
async def submit_pull_request_review(
    ...
    # content must match the one the caller was shown.
    if content_digest and content_digest != pending.get("contentDigest"):
        raise ValueError(
