...
class PRWatcherRegistry:
    ...
    def _run_agent_pass(
        ...
        if not self._export_is_durable(st, clone, attempt):
            # The clone's origin is dead, so the queue patch is the ONLY durable copy of
            # this pass's commits. Mark the directory for RETENTION rather than letting the
            # `finally` delete verified work over a filesystem hiccup.
