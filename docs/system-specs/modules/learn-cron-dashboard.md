...
- **The dashboard-notification-only path does NOT advance it.** The bell is
  ...
  suppress a result nobody has seen. A configured Slack client is not itself a
  destination: when a job has no pinned channel, creator/owner DM candidate, or
  non-Slack origin, result delivery is terminally skipped without claiming or
  persisting an outbox. Later schedules therefore keep executing fresh turns.
  A failure after a real destination candidate is found remains retryable.
...
- **Update handoff and result outbox**: if a script or agent child cannot launch because both the running interpreter and the loaded Kiro Crew package tree have vanished, the gateway treats that wake as a retryable skip; the replacement gateway retries on the next tick. Both paths must be absent, so an unrelated missing working directory, sandbox wrapper, or user binary remains a real failure. A never-started launch leaves the existing result/outbox generation byte-identical. Every cron record carries a durable execution generation. Only execution-owned state advances it: result/outbox and delivery ownership, schedule progress, retry/failure and auto-pause state, and owed-fire debt. Configuration, acknowledgement, delivery-destination and session-owner edits do not advance it. Each run executes from a detached snapshot of the canonical registry record, so a concurrent config save cannot serialize the run's partial execution state. Result validation refreshes presentation and routing from the current canonical record; if destination or owner changes before claim, claim refuses without writing an outbox and the gateway resolves the new route before retrying. Each run snapshots the generation plus its schedule and pause fields before execution. Result validation, ownership, multipart checkpoints, settlement, and the final runtime merge compare-and-swap execution-owned state while starting from the latest record, so a benign concurrent rename/ack/model/destination/owner edit composes with the run instead of dropping its progress. The merge never copies those configuration fields back. A completed at-job may park itself only while its snapshotted schedule and pause fields are unchanged; a concurrent reschedule or pause/resume wins. Conversely, a newer result, delivery checkpoint/settlement, auto-pause, retry state or owed-fire transition advances the execution generation and rejects the stale runtime wholesale. Deletion remains authoritative because a missing row is never recreated. Message-cron results move through explicit terminal dashboard-only,
persisted pending/unacknowledged, acknowledged/unsettled, and settled states. App
ownership provenance (`created_by="app:…"`) is not a Slack recipient: only a
validated Slack user id may be opened as a DM, with a valid install owner as the
fallback. A missing destination or a durable cron-surface channel-policy refusal
finishes the result as dashboard-only without an external dedup tombstone or
outbox debt; a real transport failure remains pending. Retry never reruns
provider/tool work while bytes remain pending; successful settlement continues
into the scheduled agent occurrence that triggered the retry. A retained
`delete_after_run` one-shot is the opposite terminal: pending or acknowledged
bytes suppress provider work until settlement, then the same execution returns
directly to the merge-owned removal path instead of dispatching the one-shot a
second time. Cancellation during validation or claim drains the store
transaction, persists the exact completed generation as pending, and releases
local ownership before propagating. A dashboard-only generation is published and its owed-fire debt cleared in one locked save even when no external-delivery claim was needed; a hard kill before the later runtime merge therefore cannot resurrect the occurrence. Multipart Slack delivery snapshots the rendered part list and concrete destination before the first send, then persists the completed-part cursor plus parent/thread identity after every acknowledged part. Route existence is independent of client readiness: a pinned channel or valid DM candidate stays pending while the Slack client is unavailable instead of becoming dashboard-only. Each part also carries a deterministic UUID `client_msg_id` derived from job, result generation, and part index. A replacement retries the same bytes with the same Slack identity when the process dies after `chat.postMessage` accepts a part but before the cursor save. This relies on Slack's server-side `client_msg_id` duplicate contract rather than pretending a local post-then-save sequence can prove exactly-once delivery by itself. Later destination or owner edits remain canonical configuration but cannot redirect the already-started multipart generation. A restart resumes at the exact next part, retains the same parent, and full-result acknowledgement is recorded only after every part succeeds; partial failure or cancellation leaves the remaining outbox and owed occurrence durable while the live-owner claim excludes a competing sender. Confirmed whole-result settlement remains the only transition that clears pending bytes, multipart progress, and owed debt. The draining gateway deliberately does not rebind the child to the newly promoted interpreter: its in-memory launcher contract and package paths belong to the old version, and mixing those with a new-version child is less safe than one delayed wake.
