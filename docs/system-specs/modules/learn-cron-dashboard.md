...
Every Research Lab finding-content read uses the same descriptor-bound gate as
the file tools (`hooks.safe_read_file_bytes_nolink`). Cycle discovery accepts
only a single-linked regular `cycle*.json` leaf beneath the exact absolute
`<research>/<campaign>/findings/` shape and refuses a linked/junction campaign,
findings directory, or leaf; the cumulative `FINDINGS.md` report uses the same
gate. The opened inode must remain inside that campaign and must not resolve to a
sensitive path. The dashboard report view and grill tree are capped at 1 MiB;
an oversized report view is truncated with a notice and bounded recent-cycle
evidence so it remains useful without becoming an unbounded response.

Artifact and Knowledge exports use a separate complete-report mode through that
same campaign-owned reader. It accepts up to 4 MiB without truncation and never
injects the dashboard-only notice or recent-cycle evidence into authoritative
export content. Four MiB leaves worst-case HTML-escaping headroom below the
Artifact Store's 25 MiB content ceiling. Both export routes return
`findings_missing`/404 when the leaf is absent, `findings_refused`/409 when a
present leaf fails the no-link/ownership gate, and `findings_too_large`/413 when
the complete report exceeds the export bound. Cycle, dashboard, stagnation,
run-snapshot, export, and grill-tree output retains its existing recursive
credential and exfiltration-URL redaction. URL strings inside finding JSON remain
data and are never decoded or reinterpreted as local paths.

Research Lab Resume clears old worker-done marker evidence while holding the campaign transition lock, but it does not delete the prior inactive AutoNudge record. `_launch_loop` hands the old and new records to `AutoNudgeService.add()` as one replacement transaction. Both agent and workflow launch helpers return `True` only after a durable loop/run identity exists. Missing services, vanished campaign rows, start exceptions, and missing workflow run IDs return `False`; the enclosing transaction treats that as failure and restores the prior campaign instead of returning `RUNNING`. Workflow start registers its run before local `workflow_run.json` publication, so a failed identity write first cancels that exact returned run ID and then re-raises into campaign rollback; a duplicate autonomous run cannot escape unowned. The new loop becomes visible only after its snapshot is durable; a failed write restores the old inactive loop and its stop reason, and the Start/Resume transaction restores the exact prior campaign status, timestamps, error, and run-finding baseline before releasing the transition lock. Preparation, RUNNING publication, worker launch, and conditional rollback run in one child task; request cancellation waits for that whole task to settle while the outer campaign lock remains held. A replacement that commits keeps RUNNING, while any exception from RUNNING publication (including a post-SQLite sidecar failure) or worker replacement restores the exact prior row. Resume therefore cannot publish a zombie RUNNING campaign, lose its recovery record, or let a concurrent delete race a late status write. A cycle or runtime bound that expires during a dispatched dashboard turn remains inactive but unsettled until the shared running-or-between-stages predicate clears, allowing the final finding to decide COMPLETE versus STOPPED/FAILED before the loop record is consumed. The settlement transaction snapshots the exact loop generation plus every terminal-bound input (active state, stop reason, cycle cap/count, runtime budget, creation anchor, and derived deadline) before its off-loop database and finding bookkeeping. At the final boundary under the same campaign transition lock it re-reads the exact live generation after each suspension and classifies only from that complete current snapshot: a lifted bound aborts the stale terminal request, a newly spent or opposite bound is reclassified with runtime/cycle precedence intact, and a replacement generation aborts without falling through to stagnation. A missing loop retains the campaign-row fallback for compatibility.
...
The AutoNudge service refuses `active=true` with a conflict (HTTP 409 at the
transport boundary) while a retained cycle cap or runtime budget is still spent.
The Research Lab watchdog treats that conflict as lifecycle evidence before its
generic app-enable re-arm. A cycle-cap stop becomes COMPLETE only when the number
of readable persisted cycle findings reaches the cap that stopped the loop. A
missing or malformed finding keeps the conservative FAILED verdict; settlement
removes only the inactive loop, preserves every finding file, and FAILED remains
resumable. A runtime-budget stop becomes STOPPED with its findings preserved.

Fresh Research Lab starts/resumes remove any inactive prior-run loop plus marker
evidence before publishing RUNNING. This consumes tombstones and spent-bound
records from the prior run without deleting its findings. Loop removal precedes
the potentially unbounded off-thread marker cleanup, so the watchdog can never
observe a half-prepared new run. Direct `_launch_loop` callers preserve the same
cleanup order before any service/state availability early-return.
