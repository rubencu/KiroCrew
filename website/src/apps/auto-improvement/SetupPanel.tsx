        ...
        <div className="mt-3 flex flex-col gap-2">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              saveConfig.mutate({ branch: e.target.value })
            }}
            className="rounded border border-border bg-card px-2 py-1 text-[13px]"
            aria-label={i18nT('autoImprovement.branchLabel')}
          >
            <option value="">{i18nT('autoImprovement.branchDefault')}</option>
            {/* Guarantee the CONFIGURED branch is always a selectable option. A
                <select> whose value matches no <option> renders as the first entry,
                which would silently misreport the branch a run targets — so if the
                branches list has not yet produced the persisted value, prepend it. */}
