...
export default function DevFleetPage() {
  ...
  // A failed restart is the one error on this page whose message is a set of
  // commands to retype. Toasts are pointer-events:none and self-dismiss, so they
  // cannot be selected or copied and long paths vanish mid-read — keep the text
  // on the page until it is dealt with.
  ...
  async function makeLive(w: Worktree) {
    ...
    try {
      ...
      if (!r?.ok) {
        ...
        // restart_detached's wedge message, whose whole value is two absolute-path
        // commands the operator has to retype — useless in a 7s toast.
