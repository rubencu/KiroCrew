...
import { Badge, Btn, Checkbox, IconButton, Input, SearchInput, Skeleton } from '../components/ui'
...
export default function WebhooksPage() {
  ...
  return (
    // `min-w-0` is load-bearing: this element is itself a flex ITEM of the
    // dashboard shell, and a flex item defaults to `min-width: auto`, i.e. it
    // refuses to shrink below its min-content width. Without it the rail plus
    // detail pane forced ~846px regardless of the viewport, so on a phone the
    // whole page overflowed and the detail side was clipped rather than laid
    // out narrow. `w-full` then keeps it filling the shell on desktop.
    <div className="flex h-full w-full min-w-0 min-h-0 bg-bg text-text">
      {rail.collapsed
        ? (
          <aside
        ...
        : (
          ...
          >
            <div className="h-[46px] shrink-0 flex items-center gap-2.5 px-3 border-b border-border">
              ...
              <span className="text-[13px] font-semibold text-text-strong">{i18nT('pages.webhooksPage.webhooks')}</span>
              <span
      ...
      <main className={`flex-1 min-w-0 min-h-0 flex-col bg-bg ${mobileRailOpen ? 'hidden' : 'flex'}`}>
        <div className="h-[46px] shrink-0 flex items-center gap-2 px-4 border-b border-border bg-bg-accent">
          <div className="flex items-center gap-1.5 text-[12px] min-w-0">
            <span className="text-muted hidden sm:inline">{i18nT('pages.webhooksPage.webhooks')}</span>
            <span className="text-muted-strong hidden sm:inline">/</span>
            <span className="text-text-strong font-semibold truncate" data-testid="webhook-detail-title">{leaf}</span>
          </div>
          {/* Below `sm` the two action labels do not fit beside the breadcrumb —
              they wrapped inside the fixed-height bar and were clipped. Drop to
              icon-only there, keeping the accessible name on the button so the
              action stays identifiable; the breadcrumb prefix goes too, since the
              page title is already the surface the user is on. */}
...
function ModeSwitch({ mode, onChange }: {
  ...
  return (
    <span
      ...
      data-mode={mode}
    >
      {(['signed', 'bearer'] as const).map(m => (
        <button
          key={m}
          type="button"
          data-testid={`webhook-example-mode-${m}`}
          aria-pressed={mode === m}
          onClick={() => onChange(m)}
          className={`px-2 py-1 text-[12px] transition-colors ${
            mode === m ? 'bg-accent text-accent-fg' : 'text-muted hover:text-text hover:bg-bg-hover'
          }`}
        >
          {m === 'signed' ? i18nT('pages.webhooksPage.signed') : i18nT('pages.webhooksPage.bearer_only')}
        </button>
      ))}
    </span>
  )
