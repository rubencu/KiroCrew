...
describe('AboutPanel gateway update check', () => {
  ...
  it('offers the installer command instead of a broken Update button on a wheel install', async () => {
    const command = 'curl -fsSL https://download.crew.kiro.dev/cli.sh | sh -s -- --channel insider'
    stubFetch({
      checked: true,
      available: true,
      error: '',
  ...
  it('copying the command flips the button label', async () => {
    const command = 'curl -fsSL https://download.crew.kiro.dev/cli.sh | sh -s -- --channel stable'
